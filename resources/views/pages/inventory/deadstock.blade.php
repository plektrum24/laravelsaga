        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="text" x-model="filters.search" @input.debounce.500ms="fetchDeadstock()"
    ...
    <div class="max-w-8xl mx-auto">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div x-show="isLoading" class="py-16 text-center">
                <div class="animate-spin rounded-full h-16 w-16 border-4 border-brand-200 mx-auto"></div>
                <p class="text-gray-500 dark:text-gray-400 mt-4">Loading deadstock data...</p>
            </div>
        </div>
    </div>
</div>
...
function deadstockPage() {
    return {
        ...
        exportData() {
            const token = localStorage.getItem('saga_token');
            const params = new URLSearchParams(this.filters);
            window.open('/api/products/deadstock/export?' + params, '_blank');
        },
        ...
        bulkRestock() {
            ...
            Swal.fire({
                icon: 'info',
                title: 'Bulk Restock',
                text: `Restock ${this.products.length} deadstock products?`,
                ...
                if (result.isConfirmed) {
                    ...
                    console.log('Bulk restock:', this.products.map(p => p.id));
                    Swal.fire({
                        icon: 'success',
                        title: 'Restock Initiated',
                        text: 'Creating purchase orders...',
                    });
                }
            });
        },
        ...
        createPromotion(product) {
            Swal.fire({
                ...
                html: `
                    <p class="text-left mb-4">Create clearance promotion for:<br><strong>${product.name}</strong></p>
                    <div class="text-left">
                        <label class="block text-sm mb-1">Discount %</label>
                        <input type="number" id="discountPercent" class="swal2-input" placeholder="10" value="10">
                    </div>
                `,
                showCancelButton: true,
                ...
                preConfirm: () => {
                    ...
                    return { productId: product.id, discount };
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    console.log('Create promotion:', result.value);
                    Swal.fire({
                        icon: 'success',
                        title: 'Promotion Created',
                        text: `${result.value.discount}% discount applied`,
                    });
                }
            });
        }
