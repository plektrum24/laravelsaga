...
@section('content')
    <div x-data="inventoryPage()" x-init="initPage()">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <button @click="toggleScanner()"
                    ...
                    <span x-text="scannerEnabled ? 'Scanner ON' : 'Scanner OFF'"></span>
                </button>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        <tr x-show="isLoading" class="animate-pulse">
                            <td colspan="7" class="px-4 py-8 text-center text-gray-500">Loading products...</td>
                        </tr>
                        ...
                        <template x-for="product in products" :key="product.id">
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                                ...
                                <td class="px-4 py-3 text-right font-medium text-gray-800 dark:text-white align-middle"
                                    x-text="formatCurrency(getBasePrice(product))"></td>
                                <td class="px-4 py-3 text-center align-middle">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                        :class="product.stock <= product.min_stock ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'">
                                        <span x-text="formatNumber(product.stock)"></span>
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400 align-middle"
                                    x-text="getUnitName(product.base_unit_id)"></td>
                                <td class="px-4 py-3 text-center align-middle">
                                    <div
                                        class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button @click="openEditModal(product)"
                                            class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg dark:hover:bg-blue-900/30 transition-colors"
                                            title="Edit">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <button @click="deleteProduct(product.id)"
                                            class="p-1.5 text-red-600 hover:bg-red-50 rounded-lg dark:hover:bg-red-900/30 transition-colors"
                                            title="Delete">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div x-show="showScannerModal"
            class="fixed inset-0 z-[100000] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4" x-cloak>
            <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg overflow-hidden relative">
                <button @click="closeScanner()"
                    class="absolute top-4 right-4 z-10 bg-black/50 text-white p-2 rounded-full hover:bg-black/70">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12">
                        </path>
                    </svg>
                </button>
                <div id="qr-reader-inventory" class="w-full h-full bg-black"></div>
                <div class="p-4 text-center bg-white dark:bg-gray-800">
                    <p class="text-sm text-gray-600 dark:text-gray-300">Point camera at a barcode to scan</p>
                </div>
            </div>
        </div>
                ...
                return {
                    ...
                    toggleScanner() {
                        ...
                        if (this.scannerEnabled) {
                            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Scanner Mode: ON', showConfirmButton: false, timer: 1500 });
                        } else {
                            Swal.fire({ toast: true, position: 'top-end', icon: 'info', title: 'Scanner Mode: OFF', showConfirmButton: false, timer: 1500 });
                        }
                    },
                    ...
                    openScanner() {
                        ...
                        if (!isSecure) {
                            Swal.fire({ icon: 'warning', title: 'HTTPS Required', text: 'Scanner requires HTTPS.' });
                            return;
                        }
                        this.showScannerModal = true;
                        this.$nextTick(() => {
                            this.html5QrCode = new Html5Qrcode("qr-reader-inventory");
                            this.html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 150 } },
                                (decodedText) => {
                                    this.closeScanner();
                                    this.currentProduct.barcode = decodedText;
                                    Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Barcode: ' + decodedText, showConfirmButton: false, timer: 1500 });
                                },
                                (errorMessage) => { }
                            ).catch((err) => {
                                console.error("Camera error:", err);
                                this.showScannerModal = false;
                                Swal.fire({ icon: 'error', title: 'Kamera Error', text: 'Cannot access camera.' });
                            });
                        });
                    },
                    ...
                    async openEditModal(product) {
                        ...
                        try {
                            ...
                            if (data.success && data.data) {
                                ...
                                this.stockInputValue = this.convertStockToLargest(p.stock);
                            }
                    ...
                    formatNumber(num) {
                        return parseFloat(num) % 1 === 0 ? parseInt(num) : parseFloat(num).toFixed(2);
                    },
                    ...
                    handleImport(event) {
                        // Placeholder for import logic if needed or reused from HTML
